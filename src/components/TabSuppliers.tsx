...
export default function TabSuppliers({
  ...
  const compSups = getCompatibleSuppliers();
  const displaySups = showAllCatalog ? MOCK_CATALOG_SUPPLIERS : compSups;
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6 p-4 bg-indigo-50/50 border border-indigo-100 rounded-2xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-3 pb-2 border-b border-indigo-100 text-slate-800">
          <div className="flex flex-wrap items-center gap-2">
            <div className="p-1 px-2 bg-indigo-600 text-white rounded font-bold text-[9px] flex items-center gap-1 uppercase tracking-wider">
              👑 Sugestões de Fornecedores
            </div>
            <span className="text-xs font-bold text-indigo-900">
              {compSups.length > 0 
                ? `Identificamos ${compSups.length} parceiros ideais no catálogo para esta licitação!` 
                : showAllCatalog 
                  ? "Todas as empresas do catálogo comercial" 
                  : "Nenhuma empresa compatível identificada. Mostre o catálogo geral ao lado."}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 font-sans">
            {compSups.length > 0 && (
              <button
                ...
                className="bg-white hover:bg-slate-100 text-indigo-700 border border-indigo-200 px-3 py-1 rounded text-[10px] font-bold transition duration-150 cursor-pointer flex items-center gap-1 shadow-sm"
              >
            ...
            <button
              ...
              className="bg-indigo-100 hover:bg-indigo-200 text-indigo-800 px-3 py-1 rounded text-[10px] font-bold transition duration-150 cursor-pointer"
            >
              {showAllCatalog ? "Mostrar Apenas Compatíveis" : "Ver Catálogo Completo"}
            </button>
          </div>
        </div>

        {displaySups.length === 0 ? (
          <div className="text-center py-5 bg-white rounded-xl border border-dashed border-indigo-100 p-3">
            <p className="text-[11px] text-gray-500 font-bold">Nenhum fornecedor correspondente pré-cadastrado para este objeto.</p>
            <p className="text-[10px] text-gray-400 mt-0.5 leading-normal">
              Licitador, você pode expandir o <strong className="text-indigo-600">Catálogo Completo</strong> à direita para escolher e selecionar parceiros comerciais.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
            {displaySups.map((sup, idx) => {
              const supplierIndex = activeSuppliers.findIndex(s => s.name.toLowerCase() === sup.name.toLowerCase());
              const isAdded = supplierIndex !== -1;
              return (
                <div key={idx} className="bg-white p-3 rounded-xl border border-indigo-100/80 hover:border-indigo-300 transition shadow-2xs flex flex-col justify-between gap-2.5 font-sans">
                  <div className="space-y-1">
                    <div className="flex items-start justify-between gap-1">
                      <h4 className="text-xs font-bold text-slate-800 truncate block max-w-[70%]" title={sup.name}>
                        {sup.name}
                      </h4>
                  ...
                  <div className="flex items-center justify-between gap-2 border-t border-slate-100 pt-2 shrink-0">
                    ...
                    <button
                      ...
                      className={`py-1 px-2.5 rounded text-[10px] font-bold border transition duration-150 flex items-center gap-1 cursor-pointer ${
                        isAdded
